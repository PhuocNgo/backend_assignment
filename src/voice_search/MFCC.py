...
def extract_features(audio_file, n_fft=128, hop_length=32, n_mfcc=13):
    try:
        ...
        # Trích xuất MFCC
        mfccs = librosa.feature.mfcc(y=y_5_sec, sr=sr, n_mfcc=n_mfcc, n_fft=n_fft, hop_length=hop_length)

        # Chuẩn hóa từng hệ số MFCC theo thời gian
        mfccs_normalized = (mfccs - np.mean(mfccs, axis=1, keepdims=True)) / (np.std(mfccs, axis=1, keepdims=True) + 1e-6)
